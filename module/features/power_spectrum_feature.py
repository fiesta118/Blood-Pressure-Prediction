...
def psqi(ecg_signal: list, sampling_frequency: int) -> float:
    ...
    pds_num = [
        np.abs(yf[idx]) for idx in range(len(xf)) if xf[idx] >= 5 and xf[idx] <= 15
    ]
    pds_denom = [
        np.abs(yf[idx]) for idx in range(len(xf)) if xf[idx] >= 5 and xf[idx] <= 40
    ]
...
def bassqi(ecg_signal: list, sampling_frequency: int) -> float:
    ...
    pds_denom = [
        np.abs(yf[idx]) for idx in range(len(xf)) if xf[idx] >= 0 and xf[idx] <= 40
    ]
